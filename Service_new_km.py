...
if pp == True and ss == False:
    ...
    if st.button('Записать в таблицу'):
        now = datetime.now()
        new_row = {'Время входа':now, 'Контрагент': [text], 'Сфера применения': [''], 'ФИО, должность и телефо': [''], 'Эл. почта':[''], 'Общая почта, директор':[''], 'Примечание':[''], 'Комментарий':[''], 'Отгрузка':[''], 'Комментарии 2024-2025 год':['']}
        ...
        with pd.ExcelWriter("ЛКМ.xlsx") as writer:
