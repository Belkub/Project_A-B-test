...
st.header("chat_mobile/contacts")

df_c = pd.read_excel('РУО_1.xlsx')
print(df_c.columns)
##df_c = df_c[['Контрагент', 'Сфера применения', 'Контактное лицо и Эл. почта',
##             'Год, месяц согласования, продукт и завленный объем ', 'Примечание',
##             'Комментарий', 'Отгрузка', 'Комментарии 2024-2025 год',
##              'Текущее состояние (ноябрь 2025)']]
if 'Unnamed: 0' in df_c.columns:
    df_c = df_c.drop('Unnamed: 0', axis = 1)
#df_c = df_c.dropna(subset=['Контрагент'])
...
a = {}
pp = st.sidebar.checkbox('Сформировать сводную таблицу контактов')
if pp == True and ss == False:
    ...
    for i in list(df_c['Контрагент']):
        a[i] = i
    t_1 = st.selectbox('Выбрать контрагента: ', list(a))
    ss_1 = st.sidebar.checkbox('Изменить имя контрагента/удалить')
    dd = st.sidebar.checkbox('Заполнить/изменить поле')
    if ss_1 == True and dd == False:
        st.title('Изменение имени/удаление контрагента')
        ...
        if rr_1: 
            ...
            if text:
                with open("file.txt", "w") as file:  
                    ...
                    file.close()
                company_1 = text
        company = st.text_input('Имя контрагента: ', value = company_1)
        with open("file.txt", "w") as file:  
            file.write(company)
            file.close()
        with open("file.txt", "r") as file:  
        ...
        if st.button('Записать новое имя'):
            df_c = df_c.dropna(subset=['Контрагент'])
            df_c = df_c.drop_duplicates()
            df_c.loc[df_c['Контрагент'] == t_1, 'Контрагент'] = text
            df_c.to_excel('РУО_1.xlsx')
        ss_2 = st.checkbox('Удалить контрагента')
        if ss_2:     
    ...
    if dd == True and ss_1 == False:
        ...
        comp = list(df_c['Контрагент'])

        print(df_c.loc[df_c['Контрагент'] == t_1], 'Время входа')
        
        
        with open("file.txt", "w") as file:  
            file.write(str(df_c.loc[df_c['Контрагент'] == t_1, t_2][comp.index(t_1)]))
            file.close()
        with open("file.txt", "r") as file:  
            text_1 = file.read()
            file.close()
        ...
        if st.button('Записать поле'):
            df_c = df_c.dropna(subset=['Контрагент'])
            df_c = df_c.drop_duplicates()
            df_c.loc[df_c['Контрагент'] == t_1, t_2] = text_3
            df_c.to_excel('РУО_1.xlsx')

else:
    st.title('Создание контрагента')
    a = {}
    for i in list(df_c['Контрагент']):
        a[i] = i
    t_1 = st.selectbox('СОЗДАТЬ КОНТРАГЕНТА: ', list(a))
    ...
    with open("file.txt", "r") as file:  
        text = file.read()
        file.close()
    company = st.text_input('Название контрагента: ', value = text)
    with open("file.txt", "w") as file:  
        file.write(company)
        file.close()
    with open("file.txt", "r") as file:  
        text = file.read()
        file.close()
    w = st.checkbox('Вставка по индексу')
    if w:   
        ind = {}
        for i in range(len(df_c)):
            ind[i] = i
        ind_1 = st.selectbox('Выбрать индекс записи: ', ind)
    ##wdf_c = st.checkbox('Записать название')
    if st.button('Записать в таблицу'):
        now = datetime.now()
        new_row = {'Время входа':now, 'Контрагент': [text], 'Сфера применения': ['РУО'], 'Контактное лицо и Эл. почта': [''], 'Год, месяц согласования, продукт и завленный объем ':[''], 'Примечание':[''], 'Комментарий':[''], 'Отгрузка':[''], 'Комментарии 2024-2025 год':[''], 'Текущее состояние (ноябрь 2025)':['']}
        df_m = pd.DataFrame(new_row)
        if w:
            df_c = pd.concat([df_c.iloc[:ind_1], df_m, df_c.iloc[ind_1:]]).reset_index(drop=True)
        else:
            df_c = pd.concat([df_c, df_m], axis=0, ignore_index = True)
        df_c = df_c.drop_duplicates(subset=['Контрагент'])
        df_c.dropna(subset=['Контрагент'])
        with pd.ExcelWriter("РУО_1.xlsx") as writer:
